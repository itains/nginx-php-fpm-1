...
The container has the option for you to pass it the __SSH_KEY__ variable with a **base64** encoded **private** key. First generate your key (https://help.github.com/articles/generating-a-new-ssh-key-and-adding-it-to-the-ssh-agent/#generating-a-new-ssh-key) and then make sure to add it to github and give it write permissions if you want to be able to push code from the container. Then run:
```
base64 -w 0 /path_to_your_private_key
```
**Note:** Copy the output, but be careful not to copy your prompt

If you already have a key, you need to upgrade the container. https://github.com/itains/cloud-infrastructure/wiki/2.4.3.2.-Update-site-in-the-container#container-specific-environment-variables
